...
program.name('secretforge').description('AI-powered API key management CLI').version('1.0.0');
...
program
  ...
  .action(async () => {
    const context = await loadContext();
    const response = await fetch(`${context.apiEndpoint}/api/secrets?userId=${context.userId}`);
    const data = (await response.json()) as any;

    console.log(chalk.bold('\n🔑 Your API Keys:\n'));
    data.secrets.forEach((secret: any) => {
      console.log(`${chalk.blue(secret.service)} (${chalk.gray(secret.environment)})`);
